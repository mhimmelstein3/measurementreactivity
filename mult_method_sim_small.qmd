---
title: "Basic Measurement Reactivity Sim Small Effects"
format:
  html
editor: visual
editor_options: 
...
bias_dat <- point_df_long %>% filter(parameter == "diff" & model != "true")

lm1 <- lmer(estimate ~ model + (1 | iter), bias_dat)
anova(lm1)


pairs(emmeans(lm1, specs = "model"))

error_dat <- point_df_long %>% filter(parameter == "diff2" & model != "true")

lm2 <- lmer(sqrt(estimate) ~ model + (1 | iter), error_dat)
anova(lm2)


pairs(emmeans(lm2, specs = "model"))
